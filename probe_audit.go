...
func runAuditProbes(args []string, stdout io.Writer) error {
	...
	for i := 0; i < len(rest); i++ {
		...
		case "-p", "--provider", "--alias":
			if i+1 >= len(rest) {
				return fmt.Errorf("%s 需要一个 provider 别名", a)
			}
			only = rest[i+1]
			rest = append(rest[:i], rest[i+1:]...)
			i--
		case "-h", "--help":
			fmt.Fprintln(stdout, "用法: audit-probes [-p <provider 别名>]")
			return nil
		default:
			return fmt.Errorf("audit-probes 不接受额外参数: %s", a)
		}
	}
	...
	fmt.Fprintln(stdout, "可疑结果（HTTP 不是 401/403/2xx —— 意味着端点不认识这个协议路径或 key 处理方式不同）")
	fmt.Fprintln(stdout, strings.Repeat("=", 100))
	if len(susp) == 0 {
	...
	fmt.Fprintln(stdout)
	fmt.Fprintf(stdout, "汇总: %d 条可疑 / %d 条总计\n", len(susp), len(all))
	return nil
}
